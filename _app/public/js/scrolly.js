...
jQuery.fn.n33_scrolly = function() {      
  ...
  t.click(function(e) {
    ...
    if (h.charAt(1) == '#' && h.length > 2 && (target = jQuery(h.substring(1))).length > 0) {
...
jQuery.n33_scrollzer = function(ids, userSettings) {
  ...
  top.load(function() {
    ...
    top.scroll(function(e) {
      ...
      for (k in elements) {
        if (k != lastId &&  pos >= elements[k].start &&  pos <= elements[k].end) {
          lastId = k;
          found = true;
        }
        i++;
      ...
      if (settings.lastHack &&  pos + top.height() >= doc.height()) {
        lastId = k;
        found = true;
      }
      
      // If we found one ...
      if (found &&  !links.hasClass('scrollzer-locked')) {
...
jQuery(function() {
  ...
  // Scrollyfy links
  $nav_a.n33_scrolly().click(function(e) {
    e.preventDefault();
